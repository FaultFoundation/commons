...
|---|---|---|
| School icon fallback | `school_favicons` | Hipo school id/name, normalized name, primary domain, and its Google favicon-service URL. The scraper phrase-matches the longest normalized school name only when a provider omits its team logo; Commons never joins this table at render time. Seeded separately with `npm run db:cen:seed:favicons:*`. |
...
EXT_MATCHES(*id, event_id→EXT_EVENTS, source_match_id, scheduled_at?, state?,
            ...
            entrant_1_logo_url?, entrant_2_logo_url?, entrant_1_score?,
