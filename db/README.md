...
> **Other databases:** two more D1s sit alongside this one, joined to it only in
> application code (D1 can't JOIN across bindings):
> - **`cen-sql`** ([db/cen-schema.ts](cen-schema.ts), migrations in `drizzle-cen/`,
>   bound read-only as `CEN`) — external (start.gg/FACEIT) tournaments; see the
>   `cen-sql` projection section below.
> - **`ow-player-data`** ([db/ow-schema.ts](ow-schema.ts), migrations in
>   `drizzle-ow/`, bound read-write as `OW`) — append-only Overwatch career
>   snapshots plus the cross-provider player data (`pd_*`: external FACEIT/
>   start.gg teams + match history); see the `ow-player-data` store section
>   below.
>
> Everything else here is `website-sql`.
...
|---|---|---|
| School identity + icon fallback | `school_favicons` | Hipo-backed school id/name, normalized name, primary domain, and its Google favicon-service URL, plus narrowly curated verified omissions. The scraper resolves an entrant in three passes: a leading canonical school-name phrase, then a source-controlled high-confidence collegiate alias phrase, then completion of a name that TRUNCATES a canonical one with a single generic institution tail ("Mount Vernon Nazarene" → Mount Vernon Nazarene University). Each pass also retries the entrant with unambiguous abbreviations expanded (`uni`/`univ` → university, `coll` → college), since providers write "Manchester Uni." where the seed holds the full name — entrant-side only, so this column stays as generated. Ambiguous acronyms, single-token names, and completions that fit two different schools remain unmatched. `favicon_url` asks Google for the host the institution publishes (usually `www.`), which the bare domain often answers with a generic globe; `domain` stays bare. It stamps the resolved canonical name/domain onto entrant rows even when provider art wins, and uses the favicon only when a provider omits its team logo. Commons never joins this table at render time. Seeded separately with `npm run db:cen:seed:favicons:*`. |
| External tournament catalog | `ext_tournaments` | Source id, name/game, start/end window, location, native URL, `banner_url` (cover art) + `description` (blurb — start.gg's About widgets or FACEIT's description) for the branded tile/detail hero, plus About-panel facts collected from the provider page: `contact`/`contact_type` (start.gg `primaryContact` — a Discord/email/URL), `stream_url` (a watch link — start.gg's first `streams` entry or FACEIT's active stream), `registration_closes_at` (start.gg `registrationClosesAt` / FACEIT `subscription_end`), `prize_pool` (display string — FACEIT `total_prizes` / start.gg cash `payoutTotal`), `video_url` (start.gg VideoWidget), `organizer`/`organizer_url` (start.gg `owner` / FACEIT organizer), `links_json` (JSON `[{label,url}]` — start.gg `links` / FACEIT organizer socials), `images_json` (JSON `[{url,caption}]` — start.gg ImageWidgets), `about_layout` (JSON `[{columns:[[widget,…],…]}]` — start.gg's About-tab layout preserving its 1–3 column rows of markdown/image/video widgets, so the branded view mimics the source; markdown widgets carry inline HTML the renderer folds in) |
| Provider bracket/event | `ext_events` | Tournament parent, provider event id/state, entrant count |
| Public match calendar + bracket | `ext_matches` | Event parent, provider match id, optional schedule/state/round, `round_order` (signed: +winners/−losers, for column order) + `order_key` (within-round bracket position), two entrant labels + optional provider/fallback logo URLs and their independently resolved canonical `entrant_*_school_name`/`entrant_*_school_domain`, `entrant_1_score`/`entrant_2_score`, `winner` (1/2/null), `prereq_1_id`/`prereq_2_id` (source match id feeding each slot — the feed graph for connectors), `phase_id`/`phase_name`/`phase_order` (the independent bracket a set belongs to — a start.gg event can hold several, e.g. "Round 1 Bracket" + "Round 2 Bracket"; the branded view groups by phase so each renders separately instead of mashing), `phase_group_id`/`phase_group_name`/`phase_group_order` (the pool one level below the phase — a single phase can run several disjoint pool brackets "A1".."A4" sharing one `phase_id` and identical round names; the view splits a phase into pools by these, or infers them from the feed graph's connected components when null), native match-page URL. **cen-sql's production schema is migrated by the scraper repo's `migrations/` (its `db:migrate:remote`), not the Commons' `drizzle-cen/` — the latter is the local/mirror copy; keep both column-compatible.** |
| Entrant roster + final results | `ext_standings` | Event parent, entrant label, optional provider/fallback `entrant_logo_url`, independently resolved `entrant_school_name`/`entrant_school_domain`, type and placement. Before start.gg publishes standings, attendee rows are stored with a null placement and the Commons labels the view **Entrants**; published or bracket-derived placements take precedence once available. |
...
```text
SCHOOL_FAVICONS(*id, name, normalized_name, domain, favicon_url)
...
EXT_MATCHES(*id, event_id→EXT_EVENTS, source_match_id, scheduled_at?, state?,
            ...
            entrant_1_logo_url?, entrant_2_logo_url?,
            entrant_1_school_name?, entrant_1_school_domain?,
            entrant_2_school_name?, entrant_2_school_domain?, entrant_1_score?,
            entrant_2_score?, winner?, prereq_1_id?,
            prereq_2_id?, url?)
EXT_STANDINGS(*id, event_id→EXT_EVENTS, entrant_name, entrant_logo_url?,
              entrant_school_name?, entrant_school_domain?, is_team, placement?)
```
...
## `ow-player-data` store

A third D1 ([db/ow-schema.ts](ow-schema.ts), migrations in `drizzle-ow/`, bound
...
| Feature | Relation | Shape |
|---|---|---|
| Player registry (who to poll) | `ow_players` | PK `user_id` (the Commons user id), `battletag` + OverFast `player_id`, `platform`, cached `visibility` (`public`/`private`/`not_found`/`unknown`) + `visibility_checked_at`, `poll_chunk` (0–23, `hash(user_id)%24` — the hourly bucket the poller picks it up in), `last_snapshot_at`, `first_connected_at` |
| Career snapshots (append-only history) | `ow_snapshots` | `user_id` + `captured_at` (the composite index), identity (`battletag`/`player_id`/`platform`), summary (`endorsement_level`, `title`, `avatar_url`, `namecard_url`), per-role comp ranks (`{tank,damage,support,open}_division` + `_tier`, `comp_season`), denormalized headline aggregates (`games_played`/`won`/`lost`, `time_played`, `winrate`, `kda`, `total_*` + `avg_*` for elims/assists/deaths/damage/healing) **for cheap charting**, plus `summary_json` + `stats_json` blobs for the full per-role/per-hero detail |

The headline aggregates are columns so the progress charts query a scalar per
snapshot without parsing a blob; the blobs carry the long tail (40+ heroes) so a
new OverFast field is never a migration. Rows are never updated — history is the
point.

Relational form:

```text
OW_PLAYERS(*user_id, battletag, player_id, platform, visibility?,
           visibility_checked_at?, poll_chunk, last_snapshot_at?,
           first_connected_at?)
OW_SNAPSHOTS(*id, user_id, captured_at, battletag?, player_id?, platform?,
             endorsement_level?, title?, avatar_url?, namecard_url?,
             comp_season?, {tank,damage,support,open}_division?, _tier?,
             games_played?, games_won?, games_lost?, time_played?, winrate?,
             kda?, total_*?, avg_*?, summary_json?, stats_json?)
```
...
### Cross-provider player data (`pd_*`)

The same database also carries the **cross-provider player-data** tables:
external team memberships and full match history pulled from **FACEIT /
start.gg / Challonge** for each member who linked those accounts. The Teams tab
shows `pd_teams` inline with internal teams; the Statistics → Match Data tab
reads `pd_matches`. Same two-writer model as the OW tables (Commons syncs
lazily on page open / refresh icon; the `ow-data` cron syncs a chunk of due
members hourly), with one asymmetry: **Challonge rows sync Commons-side only**,
because they read through the member's OAuth token, which only Better Auth can
mint. Only Challonge contributes no teams — it has no persistent team concept.

| Feature | Relation | Shape |
|---|---|---|
| Sync registry (who/when/where-to) | `pd_sync` | Deterministic PK `user_id:provider`, unique `(user_id, provider)`; mirrors `external_id` + `handle` out of `platform_identities` so the poller runs without a `website-sql` binding; `meta` JSON (start.gg player id, FACEIT games), `poll_chunk` (same `hash(user_id)%24` bucketing), `last_synced_at` (the TTL gate), `backfill_cursor` JSON + `backfill_done` (the unbounded-history walk), `status` (`ok`/`private`/`not_found`/`error` — surfaced to the member) + `status_detail` |
| External teams (shared across members) | `pd_teams` | Deterministic PK `provider:external_team_id` (also the `/teams/<id>/` route id), unique `(provider, external_team_id)`; `name`, `game` (provider's display name), `logo_url`, `url` (provider page), `roster_refreshed_at` (roster rewrites are budgeted per tick) |
| Provider rosters | `pd_team_members` | `team_id` → `pd_teams.id`; provider handles (`player_external_id`, `handle`, `avatar_url`) and a normalized `role` (`leader`/`captain`/`member`). Rewritten atomically per team in one `db.batch` |
| Member ↔ team links | `pd_team_links` | Deterministic PK `user_id:team_id`; the member's membership set per provider, rewritten per sync. Also the **authorization** to open an external team's detail page |
| Match history | `pd_matches` | Upsert key unique `(user_id, provider, external_match_id)`; parsed columns only, no raw payload (the backfill is unbounded): `game`, `competition_name`, `round_text`, the member's side (`team_external_id` — attributes a match to a `pd_teams` row — + `team_name`), opponent, `score_for`/`score_against`, `result` (`win`/`loss`/`draw`/null), normalized `status`, `started_at`/`finished_at`, provider `url`. Indexes: `(user_id, started_at)` for the Match Data tab, `(provider, team_external_id, started_at)` for team detail |

Relational form:

```text
PD_SYNC(*id, user_id, provider, external_id, handle?, meta?, poll_chunk,
        last_synced_at?, backfill_cursor?, backfill_done, status?,
        status_detail?)
PD_TEAMS(*id, provider, external_team_id, name, game?, logo_url?, url?,
         roster_refreshed_at?)
PD_TEAM_MEMBERS(*id, team_id, player_external_id?, handle?, role?, avatar_url?)
PD_TEAM_LINKS(*id, user_id, team_id, provider)
PD_MATCHES(*id, user_id, provider, external_match_id, game?, competition_name?,
           round_text?, team_external_id?, team_name?, opponent_team_id?,
           opponent_name?, score_for?, score_against?, result?, status,
           started_at?, finished_at?, url?)
```

Two members on the same external team share one `pd_teams` row (+ roster); two
members in the same match each keep their own `pd_matches` row (reads stay a
single `WHERE user_id = ?`; team views dedupe by `external_match_id` in app
code). The `ow-data` repo's mirror copies now also include
`lib/player-data-shared.ts` + `lib/player-data-sync.ts`. See the root
`CLAUDE.md` ("Cross-provider player data").
...
| 8 — tournaments (Challonge-backed) | `tournaments`, `tournament_participants`, `tournament_brackets`, `tournament_list_cache`, `external_matches` |
...
| App concept | Table | Key mappings |
|---|---|---|
| Roster unit | `teams` | college-affiliated (`college_id`) or ad-hoc; `game_id` is the game the team competes in (the card's corner mark); `color` is the member-chosen accent hex (card/hero stripe + logo ring; NULL = brand default — see `lib/teams-shared.ts` `teamColor`); `region`/`timezone` prefilled at creation from the creator's verified college and browser zone; `logo_url` is the `/api/avatars/…` path rather than an absolute URL, so it survives the domain cutover; `disbanded_at` is a **soft delete** — every team query filters `disbanded_at IS NULL`, which also frees the name (hence no unique index on `name`) |
...
| Cached tournament LIST | `tournament_list_cache` | ONE row (`id` is always `"default"`): `payload` (the enriched `TournamentListEntry[]` the Tournaments tab, the Series tab and the Home board all render), `built_at`, and `lease_until` (the rebuild lease). One level up from `tournament_brackets` and the same seam: built live the list costs ~2.4k `ext_tournaments` rows incl. ~1.4MB of description prose, an `ext_events` group-by, a ~32k-row aggregate join over `ext_matches` and a full pass of the discovery classifier's regexes — per request, on `/home/` (the landing page), `/tournaments/` and `/series/`. The output is identical for every member (`enrichDiscovery` reads only the global `discovery_*` overlay; the per-member `follows` is a separate query), so it is built once and shared. Rebuilt lazily past a 10-minute TTL by whichever reader wins `lease_until` — the rest serve the stale copy rather than stampeding cen-sql — and dropped immediately by the admin tournament actions. Every failure (no row, bad JSON, D1 error) degrades to building live |
...
TEAMS(           *id, program_id→PROGRAMS, game_id→GAMES?, college_id→COLLEGES?,
                 name, tag?, description?, color?, region?, timezone?,
                 discord_invite_url?, logo_url?, created_by_user_id→USER?,
                 disbanded_at? )
