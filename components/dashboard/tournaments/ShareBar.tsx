...
export function ShareBar({
  ...
  // Copy the message AND the link, so pasting it (e.g. into Discord) drops in the
  // text and unfurls the preview from the link.
  async function copy() {
    try {
  ...
  return (
    <div className="ff-share" role="group" aria-label="Share this tournament">
      ...
      {/* Discord + Instagram have no web share intent, so they copy the message
          + link to paste — Discord unfurls the preview, which now carries the
          message via Open Graph. */}
      <button
        className="ff-share__icon"
        type="button"
        onClick={copy}
        title="Copy message + link for Discord"
        aria-label="Copy message and link to share on Discord"
      >
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" aria-hidden="true">
          <path d={GLYPH.discord} />
        </svg>
      </button>
      <button
        className="ff-share__icon"
        type="button"
        onClick={copy}
        title="Copy message + link for Instagram"
        aria-label="Copy message and link to share on Instagram"
      >
